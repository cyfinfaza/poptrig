<script>
  ...
  if (tgMode)
    fetch(`/api/tg_play/highscore?u=${tgMode.u}&imref=${tgMode.imref}`)
      .then((res) => res.json())
      .then((res) => {
...
<main class={menuOpen ? "menuOpen " + menuOpen : ""} style={tgMode ? "--accent: #32c3ff" : ""}>
  <div class="game">
    ...
    <Keypad on:input={({ detail: { content } }) => (texInputs = [...texInputs, content])} on:backspace={() => (texInputs = texInputs.slice(0, -1))} on:submit={submitAnswer} />
